...
class Object:
    def __init__(self, v, f, velocities=None, forces=None):
        ...
        self.f = f
        if not velocities:
            self.velocities = np.zeros_like(v)
        else:
            self.velocities = velocities
        if not forces:
            self.forces = np.zeros_like(v)
        else:
            self.forces = forces
...
v, _, _, f, _, _ = igl.read_obj(
    "/Users/timurkesdogan/Desktop/simulation/project/bowling-simulation/assets/simple_pin.obj"
)
pins = []
for i in range(2):
...
v, _, _, f, _, _ = igl.read_obj(
    "/Users/timurkesdogan/Desktop/simulation/project/bowling-simulation/assets/simple_ball.obj"
)
v = v + np.array([-10, 0, 0])
